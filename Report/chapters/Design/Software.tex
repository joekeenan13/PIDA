...
Since this is a sequential action still, conflicts will not occur and there does not need to be any extra protections for accessing the same data or variable at the same time.
...
Every task and sensor is a class that have individual constructors depending on the needs of the task.
...
\paragraph{}
Additionally, not every family of microcontroller from STM receives the same support or even the sample supported functions from the Arduino framework.
...
If the writes an flush are completed successfully and logging is false, the next state is Close.
...
If the UART initializes successfully, the next state is the Wait state.
